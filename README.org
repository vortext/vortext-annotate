 ...
 *Note*: if after installing the modules you still get errors that they can't be found check your NODE_PATH,
 setting it explicitly in =.bashrc= with =export NODE_PATH=$NODE_PATH:/usr/local/lib/node_modules= might be necessary.
 ...
 DEV=true lein trampoline run start # will run in debug mode
   #+END_SRC
