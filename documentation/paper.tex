...
As an example, here we consider the task of bias assessment, in which a reviewers assess, e.g., whether study participants and personnel were properly blinded.
...
We have demonstrated the use of this system within the context of \ac{ebm} by automatically extracting potential risks of bias (and supporting sentences) via state-of-the-art ML methods.
...
To increase the generality of the tool we are developing a pluggable system for processing topologies, allowing developers to quickly plug in new ML systems for automated \ac{pdf} annotation.
